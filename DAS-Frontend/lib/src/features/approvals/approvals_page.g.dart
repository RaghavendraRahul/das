// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'approvals_page.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$apiNewProjectsHash() => r'6471dc7fe5d692c4c0dda5d7487c88372f2b37c5';

/// See also [apiNewProjects].
@ProviderFor(apiNewProjects)
final apiNewProjectsProvider =
    AutoDisposeFutureProvider<List<Map<String, dynamic>>>.internal(
  apiNewProjects,
  name: r'apiNewProjectsProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$apiNewProjectsHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ApiNewProjectsRef
    = AutoDisposeFutureProviderRef<List<Map<String, dynamic>>>;
String _$apiProjectClosuresHash() =>
    r'0cc62d43babb494912bcda3b4da4003f9c7a8f28';

/// See also [apiProjectClosures].
@ProviderFor(apiProjectClosures)
final apiProjectClosuresProvider =
    AutoDisposeFutureProvider<List<Map<String, dynamic>>>.internal(
  apiProjectClosures,
  name: r'apiProjectClosuresProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$apiProjectClosuresHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ApiProjectClosuresRef
    = AutoDisposeFutureProviderRef<List<Map<String, dynamic>>>;
String _$apiNewTasksHash() => r'2f212771784bd044c05e13452f06992dcab02985';

/// See also [apiNewTasks].
@ProviderFor(apiNewTasks)
final apiNewTasksProvider =
    AutoDisposeFutureProvider<List<Map<String, dynamic>>>.internal(
  apiNewTasks,
  name: r'apiNewTasksProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$apiNewTasksHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ApiNewTasksRef
    = AutoDisposeFutureProviderRef<List<Map<String, dynamic>>>;
String _$apiTaskCompletionsHash() =>
    r'fbc5dd13b861d4a657c087860cb515684e2aa8a8';

/// See also [apiTaskCompletions].
@ProviderFor(apiTaskCompletions)
final apiTaskCompletionsProvider =
    AutoDisposeFutureProvider<List<Map<String, dynamic>>>.internal(
  apiTaskCompletions,
  name: r'apiTaskCompletionsProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$apiTaskCompletionsHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ApiTaskCompletionsRef
    = AutoDisposeFutureProviderRef<List<Map<String, dynamic>>>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
