                          ...
                          Expanded(
                            child: Column(
                              children: [
                                ...
                                // Expanded sub-section for Donut centering
                                Expanded(
                                      ...
                                      : Align(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              if (widget.project.tasks
                                                  .isNotEmpty &&
                                                  widget.project.tasks.every(
                                                      (t) =>
                                                          t.task.progress >=
                                                          100)) ...[
                                                const SizedBox(height: 6),
...
class _ProjectHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Top row: accent stripe + title + status chip
        Row(
        ...
        const SizedBox(height: 6),
        // Status badge row — only for non-completed, non-active projects
        if (status != 'completed' && status != 'active' && status != 'working')
          Row(
            children: [
        ...
        if (project.plannedHours > 0) ...[
          Padding(
            ...
            child: _PlannedHoursBadge(hours: project.plannedHours),
          ),
          const SizedBox(height: 4),
        ],
      ],
...
class _LargeDonutSection extends StatelessWidget {
  final List<dynamic> tasks;
  final bool isDark;

  const _LargeDonutSection({required this.tasks, required this.isDark});

  @override
  Widget build(BuildContext context) {
    final totalTasks = tasks.length;
    final avgProgress = totalTasks > 0
        ? (tasks.fold<double>(0, (sum, t) => sum + (t.task.progress ?? 0.0)) /
            totalTasks)
        : 0.0;
    final percentage = avgProgress.isFinite ? avgProgress.toInt() : 0;
    final color = percentage == 100 ? Colors.green : const Color(0xFF05263E);
    final value = (totalTasks > 0 && percentage.isFinite)
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
          ...
          child: Row(
            ...
            children: [
              ...
              PopupMenuButton<void>(
                            ...
                            Container(
                              ...
                              child: CircleAvatar(
                                ...
                                backgroundImage: assignee['avatarUrl'] != null
                                    ? NetworkImage(
                                        assignee['avatarUrl']! as String)
                                    : null,
                                child: assignee['avatarUrl'] == null
                                    ? Text(
                                        UserColorService.getInitials(
                ...
                child: SizedBox(
                  height: 28,
                  width: 18.0 * assigneesList.length +
                      (extraAssignees > 0 ? 36 : 12),
                  child: Stack(
                    children: [
                      for (int i = 0; i < assigneesList.length; i++)
                        Positioned(
                          left: i * 18.0,
                          child: Tooltip(
                            message: (assigneesList[i]['name'] as String?) ??
                                'Employee',
                            ...
                            child: Container(
                              ...
                              child: CircleAvatar(
                                ...
                                backgroundColor:
                                    UserColorService.getColorForUser(
                                        assigneesList[i]['id']),
                                backgroundImage: assigneesList[i]
                                            ['avatarUrl'] !=
                                        null
                                    ? NetworkImage(assigneesList[i]
                                        ['avatarUrl']! as String)
                                    : null,
                                child: assigneesList[i]['avatarUrl'] == null
                                    ? Text(
                                        ((assigneesList[i]['name']
                                                    as String?) ??
                                                '?')[0]
                                            .toUpperCase(),
                                        style: const TextStyle(
        ...
        // Right Side: Completion Status Badge
        if (isCompleted && project.completedDate != null && dueDateVal != null)
          _CompletionStatusPill(
            diff: DateTime(
                    project.completedDate!.year,
                    project.completedDate!.month,
                    project.completedDate!.day)
                .difference(DateTime(
                    dueDateVal.year, dueDateVal.month, dueDateVal.day))
                .inDays,
          ),
      ],
    );
      ...
      return InkWell(
        onTap: _isLoading
            ...
            : () async {
                ...
                try {
                  // STRICT VALIDATION: Ensure ALL tasks are fully approved/completed
                  // This mirrors the backend logic and prevents early completion.
                  ...
                  if (unfinishedTasks.isNotEmpty) {
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Please complete and approve all tasks first.'),
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              try {
                ...
                // --- GLOBAL REACTIVITY (Fixes the "Not Working" issue) ---
                ref.invalidate(apiProjectsProvider);
                ref.invalidate(apiTasksProvider);
                ref.invalidate(projectsWithTasksProvider);
                ref.invalidate(paginatedDashboardProjectsProvider);
...
class _CollapsibleReopenedBanner extends StatefulWidget {
  final String reason;
  final bool isDark;

  const _CollapsibleReopenedBanner({required this.reason, required this.isDark});

  @override
  State<_CollapsibleReopenedBanner> createState() =>
      _CollapsibleReopenedBannerState();
}

class _CollapsibleReopenedBannerState
    extends State<_CollapsibleReopenedBanner> {
